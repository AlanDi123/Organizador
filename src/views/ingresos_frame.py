...
class IngresosFrame(tk.Frame):
    ...
    def crear_widgets(self):
        ...
        self.combo_ingreso_concepto.bind('<KeyRelease>', self.autocompletar_concepto)
    ...
    def autocompletar_concepto(self, event=None):
        """Autocompletado para el combobox de conceptos"""
        try:
            valor_actual = self.combo_ingreso_concepto.get().lower()
            ...
            opciones_filtradas = [
                opt for opt in self.conceptos_historicos 
                if opt.lower().startswith(valor_actual)
            ]
            
            # Actualizar el combobox con las opciones filtradas
            self.combo_ingreso_concepto['values'] = opciones_filtradas
            
            # Si hay una coincidencia perfecta, marcar como seleccionada
            if valor_actual in [opt.lower() for opt in self.conceptos_historicos]:
                # Buscar la opción exacta preservando mayúsculas
                for opt in self.conceptos_historicos:
                    if opt.lower() == valor_actual:
                        self.combo_ingreso_concepto.set(opt)
                        break
        except Exception as e:
    ...
    def mostrar_ingresos(self):
        ...
        try:
            ...
            instrucciones_label = tk.Label(
                tabla_frame,
                text="💡 Consejo: Selecciona ingresos con Ctrl+Click (múltiple) o Shift+Click (rango)",
            ...
            tabla.column("concepto", width=280, anchor=tk.W)
            ...
            for ingreso in ingresos_ordenados:
                ...
                tabla.insert("", tk.END, values=(id_ingreso, concepto, f"{monto:.2f}", fecha_display))
            ...
            botones_frame.pack(pady=(5, 20), fill=tk.X)
            
            # Botón de eliminar seleccionados
            def eliminar_seleccionados():
                seleccionados = tabla.selection()
                if not seleccionados:
                    messagebox.showwarning("Atención", "Debes seleccionar al menos un ingreso para eliminar.")
                    return
                
                cantidad = len(seleccionados)
                if messagebox.askyesno("Confirmar", f"¿Eliminar {cantidad} ingreso(s) seleccionado(s)? Esta acción no se puede deshacer."):
                    try:
                        for item in seleccionados:
                            valores = tabla.item(item)['values']
                            # El primer valor es el ID del ingreso
                            ingreso_id = valores[0]
                            # Eliminar por ID
                            from src.models.data_manager import eliminar_dato
                            eliminar_dato('ingresos', 'id', ingreso_id)
                            tabla.delete(item)
                        messagebox.showinfo("Éxito", f"{cantidad} ingreso(s) eliminado(s).")
                    except Exception as e:
            ...
            self.round_button(btn_eliminar)
        ...
        try:
            ...
            def actualizar_widget(widget, colores):
                try:
                    ...
                    if "Calendar" in widget_class or "DateEntry" in widget_class:
                        return
                    
                    # Intentar configurar, pero ignorar si falla
                    if isinstance(widget, tk.Label):
                        ...
                        try:
                            widget.config(bg=colores['acento'], fg='white')
                        except:
                            pass
                    ...
                    # Ignorar cualquier error al configurar un widget específico
                    pass
