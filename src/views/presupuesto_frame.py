...
class PresupuestoFrame(tk.Toplevel):
    ...
    def inicializar_datos(self):
        ...
        try:
            ...
            # Si no hay presupuesto guardado, generar uno sugerido
            ...
            try:
                self.ingresos_predichos = self.sistema_presupuesto.predecir_ingresos_futuros() or 0
            except:
                # Cálculo alternativo si falló la predicción
            ...
            try:
                ...
                print(f"Error al calcular seguimiento: {e}")
                self.seguimiento = {}
    ...
    def crear_interfaz(self):
        ...
        self.panel_izquierdo = tk.Frame(
            ...
            padx=10,
            pady=10,
            # highlightbackground=  # No soportado en este tema Linux, self.controller.colores['claro']['borde'],
    ...
    def actualizar_resumen(self):
        ...
        # Validar que presupuesto_actual es un diccionario
        presupuesto_dict = self.presupuesto_actual
        if not isinstance(presupuesto_dict, dict):
            presupuesto_dict = {}
        
        # Calcular totales con protección de tipos
        total_presupuestado = 0
        for cat in presupuesto_dict.values():
            if isinstance(cat, dict):
                total_presupuestado += cat.get('sugerido', 0)
        
        total_gastado = 0
        for g in self.gastos_recientes:
            if isinstance(g, dict):
                total_gastado += g.get('monto', 0)
    ...
    def actualizar_grafico_presupuesto(self):
        ...
        # Validar presupuesto_actual es un diccionario
        presupuesto_dict = self.presupuesto_actual if isinstance(self.presupuesto_actual, dict) else {}
        ...
        for gasto in self.gastos_recientes:
            if isinstance(gasto, dict):
                ...
                gastos_por_categoria[categoria] += gasto.get('monto', 0)
        
        # Obtener solo las top 6 categorías para el gráfico
        top_categorias = sorted(
            presupuesto_dict.items(), 
            key=lambda x: x[1].get('sugerido', 0) if isinstance(x[1], dict) else 0, 
            reverse=True
        )[:6]
        ...
        # Si no hay categorías, crear un gráfico vacío
        if not categorias:
            categorias = ['Sin datos']
            presupuestado = [0]
            gastado = [0]
        
        indice = list(range(len(categorias)))
        ancho = 0.35
        
        # Crear barras
        rects1 = ax.bar([i - ancho/2 for i in indice], presupuestado, ancho, label='Presupuestado', color=self.controller.colores['claro']['acento'])
        rects2 = ax.bar([i + ancho/2 for i in indice], gastado, ancho, label='Gastado', color=self.controller.colores['claro']['alerta'])
    ...
    def regenerar_presupuesto(self):
        ...
        if respuesta:
            try:
                ...
                try:
                    if self.winfo_exists():  # Verificar que la ventana aún existe
                        self.actualizar_contenido()
                except:
                    pass  # La ventana fue cerrada, ignorar
                ...
                try:
                    messagebox.showerror(
                        "Error",
                        f"No se pudo regenerar el presupuesto: {str(e)[:100]}"
                    )
                except:
