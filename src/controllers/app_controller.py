...
from src.views.dolar_widget import DolarFloatingWidgetRealtime
from datetime import datetime, timedelta
import calendar
import locale
from src.views.dashboard_financiero import DashboardFinanciero
import os
from src.views.presupuesto_frame import PresupuestoFrame
from src.models.ia_module import modulo_ia
from src.views.gastos_frame import GastosFrame
from src.views.ingresos_frame import IngresosFrame
from src.views.ahorro_frame import AhorroFrame
from src.models.gastos import calcular_total_gastos
from src.models.ingresos import calcular_total_ingresos
from src.models.data_manager import cargar_datos, eliminar_todos_datos, sincronizar_historiales

# Nuevos imports para modernización
...
class AppController:
    def __init__(self, root):
        self.root = root
        self.root.title("Organizador de Gastos Inteligente")
        ...
        # Cargar temas desde configuración centralizada
        self.update_colores()
    ...
    def update_colores(self):
        ...
        self.colores = {
            'claro': {
                'fondo': light_theme.fondo,
                'panel': light_theme.panel,
                'acento': light_theme.acento,
                'acento_oscuro': light_theme.destacado,
                'texto': light_theme.texto,
                'texto_suave': light_theme.acento,
                'borde': light_theme.borde,
                'exito': light_theme.exito,
                'alerta': light_theme.alerta,
                'destacado': light_theme.destacado
            },
            'oscuro': {
                'fondo': dark_theme.fondo,
                'panel': dark_theme.panel,
                'acento': dark_theme.acento,
                'acento_oscuro': dark_theme.destacado,
                'texto': dark_theme.texto,
                'texto_suave': dark_theme.acento,
                'borde': dark_theme.borde,
                'exito': dark_theme.exito,
                'alerta': dark_theme.alerta,
                'destacado': dark_theme.destacado
            }
        }
    
    def init_panels(self):
        """Inicializa los paneles principales de la aplicación"""
        # Crear panel derecho para gastos e ingresos
        ...
        # Crear panel para ahorros
        self.ahorro_panel = tk.Frame(
            self.panels_frame, 
            bg=self.colores['claro']['fondo'], 
            padx=10
        )
        self.ahorro_panel.pack(side=tk.RIGHT, fill=tk.BOTH, expand=False, padx=(10, 0))
        
        try:
            # Crear marcos para gastos
            self.frame_gastos = GastosFrame(self.left_panel, self)
            self.frame_gastos.pack(fill=tk.BOTH, expand=True)
            
            # Crear marcos para ingresos
            self.frame_ingresos = IngresosFrame(self.right_panel, self)
            self.frame_ingresos.pack(fill=tk.BOTH, expand=True)
            
            # Frame de ahorros
            self.frame_ahorro = AhorroFrame(self.ahorro_panel, self)
            self.frame_ahorro.pack(fill=tk.BOTH, expand=True)
            ...
            logger.error(f"Error durante la inicialización de componentes: {e}")
    ...
    def importar_datos_version_anterior(self):
        ...
        try:
            ...
            from src.utils.db_migration import extract_data_from_old_version, backup_database
    ...
    def crear_boton_modo(self):
        # Crear un frame para el botón en la esquina superior derecha
        if not hasattr(self, 'btn_frame'):
            # Usar el mismo color de fondo que main_frame para que sea invisible
            self.btn_frame = tk.Frame(self.root, bg=self.colores['claro']['fondo'])
            self.btn_frame.place(x=10, y=10)
        ...
        self.btn_toggle_modo = tk.Button(
            ...
            cursor="hand2",
            borderwidth=0,
            activebackground=self.colores['claro']['destacado'],
            activeforeground="white"
        )
        self.btn_toggle_modo.pack(side=tk.LEFT, padx=5)
    ...
    def crear_boton_dolar(self):
        ...
        try:
            ...
            print(f"Error al inicializar widget del dólar: {e}")
            self.dolar_widget = None
    ...
    def crear_boton_borrar_todo(self):
        # Usar el color según el modo actual
        modo = 'oscuro' if self.modo_noche else 'claro'
        color_alerta = self.colores[modo]['alerta']
    ...
    def toggle_modo(self):
        ...
        try:
            ...
            # Actualizar el frame del botón
            if hasattr(self, 'btn_frame'):
                self.btn_frame.config(bg=colores['fondo'])
            ...
            # Actualizar el botón de modo - ahora con ícono claro
            if hasattr(self, 'btn_toggle_modo'):
                self.btn_toggle_modo.config(
                    bg=colores['acento'],
                    fg="white",
                    activebackground=colores['destacado'],
    ...
    def abrir_analisis_categoria(self, categoria):
        ...
        try:
            from src.models.categoria_analisis import CategoriaAnalisis
        ...
        try:
            ...
            def borrar_datos():
                try:
                    ...
                    resultado = eliminar_todos_datos()
            ...
            # Actualizar la interfaz para que calcule el tamaño correcto
            dialogo.update_idletasks()
            
            # Ahora sí, ajustar el tamaño basado en el contenido
            dialogo.geometry("")  # Esto hace que se ajuste automáticamente al contenido
